...
function isAuthorObj(a: Comment['author']): a is AuthorObj {
  return typeof a === 'object' && a !== null && '_id' in a && 'name' in a;
}
...
export default function CommentList({ postId }: { postId: string }) {
  ...
  const userId = useMemo(() => user?._id ?? (user as unknown as { id?: string })?.id ?? null, [user]);

  const load = useCallback(
    async (p = 1) => {
      setLoading(true);
      try {
        const r = await publicApi<Paginated<Comment>>(
          `/posts/${postId}/comments?page=${p}&limit=10`
        );
        setItems(r.data);
        setPage(r.meta.page);
    ...
    <div className="mt-12">
      ...
      <div className="space-y-6">
        {loading ? (
          ...
          items.map((c, index) => {
            const author: AuthorObj = isAuthorObj(c.author)
              ? c.author
              : { _id: '', name: 'Unknown' };

            const canDelete =
              !!user &&
              (user.role === 'admin' || (userId !== null && userId === author._id));
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div
                    className={`w-12 h-12 bg-gradient-to-br ${getAvatarColor(
                      author.name
                    )} rounded-xl flex items-center justify-center shadow-lg flex-shrink-0 group-hover:scale-110 transition-transform duration-300`}
                  >
